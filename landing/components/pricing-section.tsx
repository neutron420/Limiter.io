...
import { CHECKOUT_URL, SALES_EMAIL } from "@/lib/config"
...
const TIERS: Tier[] = [
  {
    ...
    period: "/ forever",
    tag: null,
    description: "Personal and sandbox use. Rate-limited to 100 req/min.",
    features: [
      { text: "100 requests / minute limit", included: true },
      { text: "Token Bucket algorithm only", included: true },
      { text: "3 active projects max", included: true },
      { text: "3 active API keys max", included: true },
      { text: "All 5 advanced algorithms", included: false },
      { text: "Unlimited projects & keys", included: false },
    ],
  ...
  },
  {
    ...
    features: [
      { text: "10,000 requests / minute limit", included: true },
      { text: "All 5 advanced algorithms", included: true },
      { text: "Unlimited projects & keys", included: true },
      { text: "Real-time analytics dashboard", included: true },
      { text: "WebSockets log streaming", included: true },
  ...
  },
  {
    ...
    period: "",
    tag: null,
    description: "High-volume systems. Dedicated clusters. Custom rate limits.",
    features: [
      { text: "1,000,000+ requests / minute limit", included: true },
      { text: "Custom rule engine fine-tuning", included: true },
      { text: "Dedicated Redis instance cluster", included: true },
      { text: "Enterprise SLAs & support", included: true },
      { text: "Custom geo-distributed nodes", included: true },
    ...
    >
      ...
      <div className="px-5 pb-5 pt-3">
        <motion.button
          ...
          onClick={() => {
            if (tier.id === "pro") {
              // Lemon Squeezy hosted checkout (from env). Fall back to sign-up.
              window.location.href = CHECKOUT_URL || "/register"
      ...
      >
        <div className="flex flex-col gap-3">
          ...
          <p className="text-xs lg:text-sm font-mono text-muted-foreground leading-relaxed max-w-md">
            Every tier includes the Go SDK, atomic Redis evaluation, real-time analytics, and the full Limiter.io gateway API.
          </p>
        </div>
        <StatusLine />
      </motion.div>
